...
이 모듈은 특정 조합이 탐지되지 않아도 각 핵심 지표마다 다음에 볼 것 순서를 제공한다.
...
def _plan_vix(state: str) -> GuidancePlan:
    ...
    return GuidancePlan(intro, (
        _check(
            ...
            "이 추가 금리도 오르면 주식시장만의 변화라는 설명은 약해지고, 회사채 쪽 부담도 같이 커지는지 볼 필요가 있습니다.",
            "이 추가 금리가 내리면 회사채 시장 전반이 같이 나빠졌다는 설명은 약해지고, 주식시장 중심의 일시적 변화일 가능성을 더 봅니다.",
            "이 추가 금리에 뚜렷한 움직임이 없으면 현재 주식시장 변동성이 회사채 시장 전반으로 이어졌다고 보기 어렵습니다.",
        ),
        _check(
            "BBBOAS",
        ...
        _check(
            "AOAS",
            "A등급 기업에서도 같은 변화가 나타나는지 봅니다.",
            "A등급 기업의 추가 금리도 오르면 기업 자금 부담이 더 넓은 범위에서 커지고 있다는 설명을 더 지지합니다.",
            "A등급 기업의 추가 금리가 내리면 기업 전반의 자금 부담이 같이 커졌다는 설명은 약해집니다.",
            "A등급 기업의 추가 금리에 뚜렷한 움직임이 없으면 변화가 주식시장이나 신용등급 낮은 기업 쪽에 더 집중됐을 가능성을 봅니다.",
        ),
        _check(
            "CPSPREAD",
            "주식시장 변동성이 단기 기업자금시장의 신용도 차별과 같은 시기에 나타나는지 봅니다.",
            "단기자금 금리 차이도 벌어지면 주식시장 불안이 기업의 단기 자금조달 차별과 함께 나타나는지 더 확인합니다.",
            "단기자금 금리 차이가 줄면 단기 기업자금시장까지 부담이 이어졌다는 설명은 약해집니다.",
            "단기자금 금리 차이에 뚜렷한 움직임이 없으면 현재 변화가 단기 기업자금시장까지 이어졌다고 보기는 어렵습니다.",
        ),
    ))
...
def _plan_hy(state: str) -> GuidancePlan:
    if state in {"평소 수준", "보통 수준"}:
        intro = (
            "신용등급 낮은 기업의 추가 금리는 현재 큰 변화가 없습니다. "
    ...
    return GuidancePlan(intro, (
        ...
        _check(
            ...
            "A등급 기업의 추가 금리도 오르면 기업 자금 부담이 더 넓은 범위에서 커지고 있다는 설명을 더 지지합니다.",
            "A등급 기업의 추가 금리가 내리면 변화가 신용등급 낮은 기업이나 특정 업종에 더 집중됐다는 설명과 잘 맞습니다.",
            "A등급 기업의 추가 금리에 뚜렷한 움직임이 없으면 변화가 낮은 등급 기업 쪽에 더 집중됐을 가능성을 봅니다.",
        ),
        _check(
            "CPSPREAD",
...
def _plan_cycle(state: str) -> GuidancePlan:
    ...
    return GuidancePlan(intro, (
        ...
        _check(
            ...
            "2년 금리가 내리면 금리인하 기대가 두 금리의 관계 변화에 기여하는지 봅니다. 경기가 식을 우려로 금리인하 기대가 커진 것인지도 회사채 추가 금리로 구분합니다.",
...
def _plan_dgs30(direction: str, state: str) -> GuidancePlan:
    intro = (
        ...
        "30년 금리 변화는 첫 화면의 ‘30년 미국 국채금리 변화 나눠보기’에서 먼저 봅니다. 여기서는 10년 지표와 2년 금리가 같은 방향인지 참고합니다."
    )
    return GuidancePlan(intro, (
        _check(
            "DFII10",
            "물가 영향을 뺀 10년 금리가 같은 방향인지 참고합니다. 30년 금리 변화를 나눈 항목은 아닙니다.",
            "물가 영향을 뺀 10년 금리도 오르는 쪽입니다. 30년과는 다른 만기이므로 참고만 합니다.",
            "물가 영향을 뺀 10년 금리는 내리는 쪽입니다. 30년 금리와 방향이 달라도 직접 원인으로 단정하지 않습니다.",
            "물가 영향을 뺀 10년 금리에 뚜렷한 움직임이 없습니다.",
        ),
        _check(
            "BREAKEVEN",
            "10년 일반 국채와 물가연동 국채의 금리 차이를 참고합니다. 미래 물가 전망 그 자체로 보거나 30년 금리 변화에 그대로 더하지 않습니다.",
            "10년 일반 국채와 물가연동 국채의 금리 차이가 커지는 방향입니다. 30년 금리 변화는 첫 화면의 ‘30년 미국 국채금리 변화 나눠보기’에서 먼저 봅니다.",
            "10년 일반 국채와 물가연동 국채의 금리 차이가 줄어드는 방향입니다. 이 변화만으로 다른 만기의 금리 움직임을 설명하지 않습니다.",
            "10년 일반 국채와 물가연동 국채의 금리 차이에 뚜렷한 움직임이 없습니다.",
        ),
        _check(
            "TERMPREM",
            "10년 장기채 추가 보상은 참고용입니다. 30년 금리 변화를 나눈 두 항목에 더하지 않습니다.",
            "10년 장기채 추가 보상 추정치가 커지는 방향입니다. 30년 금리와 같이 움직여도 직접 원인으로 단정하지 않습니다.",
            "10년 장기채 추가 보상 추정치가 줄어드는 방향입니다. 30년 금리와 방향이 다르면 그 차이를 그대로 봅니다.",
            "10년 장기채 추가 보상 추정치에 뚜렷한 움직임이 없습니다.",
        ),
        _check(
            "DGS2",
            "같은 최근 약 1개월 기간에서 2년 금리와 30년 금리가 어떻게 움직였는지 봅니다. 장·단기 금리 움직임은 첫 화면의 ‘30년 미국 국채금리 변화 나눠보기’에서 먼저 봅니다.",
            "2년 금리도 오르는 방향입니다. 30년 금리와 어느 쪽이 더 크게 움직였는지는 같은 최근 약 1개월 기간에서 봅니다.",
            "2년 금리는 내리는 방향입니다. 30년 금리와 반대로 움직이면 장·단기 엇갈림으로 따로 봅니다.",
            "2년 금리에 뚜렷한 움직임이 없습니다. 30년 금리 움직임이 장기 쪽에 더 집중됐는지 봅니다.",
        ),
    ))
...
def _plan_dgs2(direction: str, state: str) -> GuidancePlan:
    ...
    if direction == DOWN:
        ...
        credit_down = "회사채 추가 금리가 내리면 기업 전반의 자금 부담 악화라는 설명은 약해지고 금리인하·물가 둔화 설명과 더 잘 맞습니다."
        credit_flat = "회사채 추가 금리에 뚜렷한 움직임이 없으면 기업 전반의 자금 부담 악화라는 확인 근거는 아직 약합니다."
    elif direction == UP:
        credit_up = "회사채 추가 금리도 오르면 높은 금리 예상과 기업 자금 부담이 같은 시기에 나타나는지 봅니다."
        credit_down = "회사채 추가 금리가 내리면 시장 불안보다 기준금리 예상 변화라는 설명과 더 잘 맞습니다."
        credit_flat = "회사채 추가 금리에 뚜렷한 움직임이 없으면 기업 자금 부담 악화가 2년 금리 상승을 설명하기 어렵습니다."
    else:
        credit_up = "회사채 추가 금리가 오르면 금리보다 기업 자금 부담이 먼저 변하는지 봅니다."
        credit_down = "회사채 추가 금리가 내리면 기업 자금 부담이 줄어드는 방향인지 봅니다."
        credit_flat = "회사채 추가 금리에도 뚜렷한 움직임이 없으면 금리와 기업 자금 부담 모두 강한 방향이 약합니다."
    return GuidancePlan(intro, (
        _check(
            "DGS30",
        ...
        _check(
            "BREAKEVEN",
            "10년 일반 국채와 물가연동 국채의 금리 차이가 가까운 금리 움직임과 같은 방향인지 참고합니다.",
            "10년 일반 국채와 물가연동 국채의 금리 차이가 커지는 방향입니다. 2년 금리의 직접 원인으로 단정하지 않습니다.",
            "10년 일반 국채와 물가연동 국채의 금리 차이가 줄어드는 방향입니다. 2년 금리와 방향이 달라도 그 차이를 그대로 봅니다.",
            "10년 일반 국채와 물가연동 국채의 금리 차이에 뚜렷한 움직임이 없습니다.",
        ),
    ))
...
def _plan_real(direction: str, state: str) -> GuidancePlan:
    intro = (
        ...
        "이 지표는 10년 구간을 이해하는 참고 자료입니다. 30년 금리 변화는 첫 화면의 ‘30년 미국 국채금리 변화 나눠보기’에서 먼저 봅니다."
    )
    return GuidancePlan(intro, (
        _check(
            "BREAKEVEN",
            "같은 10년 구간에서 일반 국채와 물가연동 국채의 금리 차이가 어느 방향인지 봅니다.",
            "10년 일반 국채와 물가연동 국채의 금리 차이도 커지는 방향입니다. 같은 10년 구간에서 두 움직임이 함께 나타나는지 참고합니다.",
            "10년 일반 국채와 물가연동 국채의 금리 차이는 줄어드는 방향입니다. 같은 10년 구간 안에서도 움직임이 엇갈립니다.",
            "10년 일반 국채와 물가연동 국채의 금리 차이에 뚜렷한 움직임이 없습니다.",
        ),
        _check(
            "TERMPREM",
            "10년 장기채 추가 보상 추정치를 참고합니다.",
            "10년 장기채 추가 보상이 커지는 방향입니다. 실질금리와 같이 움직여도 직접 원인으로 단정하지 않습니다.",
            "10년 장기채 추가 보상이 줄어드는 방향입니다. 실질금리와의 엇갈림을 그대로 참고합니다.",
            "10년 장기채 추가 보상 추정치에 뚜렷한 움직임이 없습니다.",
        ),
        _check(
            "DGS30",
            "30년 금리와 방향을 비교하되 서로 다른 만기이므로 참고만 합니다.",
            "30년 금리도 오르는 방향입니다. 두 만기가 같이 움직이지만 30년 금리 변화는 첫 화면의 ‘30년 미국 국채금리 변화 나눠보기’에서 확인합니다.",
            "30년 금리는 내리는 방향입니다. 서로 다른 만기의 엇갈림을 그대로 남깁니다.",
            "30년 금리에 뚜렷한 움직임이 없습니다.",
        ),
    ))
...
def build_plan(key: str, row: pd.Series | dict,
    ...
    r = pd.Series(row)
    state = state_name(str(r.get("state_code", "")), str(r.get("state_label", "확인 불가")), drop=bool(r.get("drop_flag", False)), key=key)
...
def render_state_guidance(key: str, row: pd.Series | dict,
    ...
    for idx, check in enumerate(plan.checks, start=1):
        ...
        if check.source == "cycle_state":
            ...
            result_display = {"상승": "오르는 중", "하락": "내리는 중", "보합": "뚜렷한 변화 없음", "판정불가": "확인 불가", "확인 불가": "확인 불가"}.get(result, result)
        lines += [
            "",
            f"### {idx}. {label}",
            f"**왜 보나:** {check.why}",
