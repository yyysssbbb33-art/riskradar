...
def _plan_dgs30(direction: str, state: str) -> GuidancePlan:
    intro = (
        ...
        "30년 금리 변화는 금리 탭의 ‘30년 금리’에서 먼저 봅니다. 여기서는 10년 지표와 2년 금리가 같은 방향인지 참고합니다."
    )
    return GuidancePlan(intro, (
        _check(
            "DFII10",
        ...
        _check(
            ...
            "10년 일반 국채와 물가연동 국채의 금리 차이가 커지는 방향입니다. 30년 금리 변화는 금리 탭의 ‘30년 금리’에서 먼저 봅니다.",
        ...
        _check(
            "DGS2",
            "같은 최근 약 1개월 기간에서 2년 금리와 30년 금리가 어떻게 움직였는지 봅니다. 장·단기 금리 움직임은 금리 탭의 ‘30년 금리’에서 먼저 봅니다.",
...
def _plan_real(direction: str, state: str) -> GuidancePlan:
    ...
    return GuidancePlan(intro, (
        ...
        _check(
            ...
            "30년 금리도 오르는 방향입니다. 두 만기가 같이 움직이지만 30년 금리 변화는 금리 탭의 ‘30년 금리’에서 확인합니다.",
...
def render_state_guidance(key: str, row: pd.Series | dict,
    ...
    if key in {"DGS30", "DGS2", "DFII10"}:
        lines += [
            "",
            "> 참고: `상태`는 RiskRadar의 경계 기준에 걸렸는지, `최근 추세`는 실제 약 1개월 방향을 보여줍니다. 그래서 `급격한 상승 없음`이면서도 최근 1개월에는 소폭 오를 수 있습니다.",
        ]
    lines += [
        "",
    ...
    for idx, check in enumerate(plan.checks, start=1):
        ...
        if check.source == "cycle_state":
            ...
            result_display = {"상승": "상승", "하락": "하락", "보합": "뚜렷한 방향 없음", "판정불가": "확인 불가", "확인 불가": "확인 불가"}.get(result, result)
        lines += [
            "",
            f"### {idx}. {label}",
            f"**왜 보나:** {check.why}",
