...
_BRANCHES: dict[str, tuple[str, str, str]] = {
    ...
    "BBBOAS": (
        "오르면 기업 신용 부담이 투자등급 경계선까지 나타나는지 봅니다. A등급과 단기 기업자금도 같이 움직이면 변화가 나타난 곳이 더 넓습니다.",
        "내리면 투자등급 경계선의 부담이 줄어드는 방향입니다. HY만 오르면 변화가 신용등급 낮은 기업에 더 집중됐다는 설명과 잘 맞습니다.",
        "뚜렷한 움직임이 없으면 현재 회사채 부담 변화가 BBB까지 이어졌다고 보기는 어렵습니다.",
    ),
    "AOAS": (
        "오르면 기업 자금 부담이 투자등급 경계선을 넘어 A등급 기업까지 넓어지는 방향입니다. HY와 BBB도 같이 변화가 나타나면 변화가 나타난 곳이 넓다는 설명이 강해집니다.",
...
def _aux_level_text(key: str, row) -> str:
    ...
    if key == "NFCI":
        if 40 <= float(pct) <= 60:
            return "평소 변화가 나타난 곳"
        return "평소보다 자금 사정이 어려워지는 쪽" if float(pct) > 60 else "평소보다 자금 사정이 느슨한 쪽"
    if key == "STLFSI":
        if 40 <= float(pct) <= 60:
            return "평소 변화가 나타난 곳"
...
def _aux_companion_line(row) -> str:
    ...
    direction = _DIR_TEXT.get(str(row.get("direction", "확인 불가")), str(row.get("direction", "확인 불가")))
...
def render_aux_detail(
    ...
    *,
    ...
    parts = [
        ...
        f"- **자료 변화가 나타난 곳:** {r.get('history_start') or '확인 불가'} ~ {r.get('history_end') or '확인 불가'}",
