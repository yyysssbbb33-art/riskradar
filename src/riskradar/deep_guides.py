...
def _notes(items: tuple[str, ...]) -> dict[str, str]:
    return {k: "현재 지표가 단독 움직임인지, 같은 시장 영역의 다른 지표와 함께 움직이는지 확인합니다." for k in items}
...
GUIDES: dict[str, GuideDef] = {
    "HY_BBB": GuideDef(
        ...
        _notes(("HYOAS", "BBBOAS", "AOAS", "CPSPREAD", "VIX", "NFCI", "STLFSI")),
    ...
    "BBBOAS": GuideDef(
        ...
        _notes(("HYOAS", "HY_BBB", "AOAS", "CPSPREAD", "VIX", "NFCI", "STLFSI", "DGS30")),
        ("넓은 국채금리 환경",),
    ...
    "AOAS": GuideDef(
        ...
        _notes(("BBBOAS", "HYOAS", "HY_BBB", "CPSPREAD", "VIX", "NFCI", "STLFSI", "DGS30")),
        ("FOMC와 정책 환경", "국채금리 환경"),
    ...
    "CPSPREAD": GuideDef(
        ...
        _notes(("HYOAS", "BBBOAS", "AOAS", "VIX", "NFCI", "STLFSI")),
    ...
    "DGS30": GuideDef(
        ...
        _notes(("DFII10", "BREAKEVEN", "TERMPREM", "DGS2", "T10Y3M")),
        ("FOMC와 정책 가이던스", "국채 수급과 발행 배경"),
        cautions=("30Y 같은만기 물가조정 금리·물가 관련 금리 차이 분해와 10Y Term Premium은 서로 다른 참고축입니다.",),
    ),
    "DGS2": GuideDef("미국 2년 국채금리입니다. 단기 정책금리 기대에 민감한 명목 금리입니다.", "단기금리가 장기금리와 같은 방향인지, 단기 구간만 다르게 움직이는지 확인합니다.", ("DGS30", "T10Y3M", "DFII10", "BREAKEVEN"), _notes(("DGS30", "T10Y3M", "DFII10", "BREAKEVEN")), ("FOMC 금리 결정", "정책 가이던스"), cautions=("2Y만으로 향후 FOMC 결정을 단정하지 않습니다.",)),
    "DFII10": GuideDef("물가 영향을 뺀 10년 금리입니다. 상승은 물가 관련 금리 차이를 제외한 장기 금리 압력이 커졌다는 뜻입니다.", "물가 영향을 뺀 장기금리 압력이 일반 장기금리와 함께 움직이는지 확인합니다.", ("DGS30", "BREAKEVEN", "TERMPREM", "DGS2"), _notes(("DGS30", "BREAKEVEN", "TERMPREM", "DGS2")), ("FOMC 정책 환경", "RiskRadar가 직접 분류하지 않는 성장·물가 배경"), cautions=("TIPS 유동성 등으로 물가 영향을 뺀 금리 해석에는 시장구조 영향이 섞일 수 있습니다.",)),
    "T10Y3M": GuideDef("10년 국채금리에서 3개월 국채금리를 뺀 값입니다. 낮아지면 단기금리가 장기금리보다 상대적으로 높아집니다.", "곡선이 더 역전되는지 또는 역전이 완화되는지 봅니다.", ("DGS2", "DGS30", "DFII10", "BREAKEVEN"), _notes(("DGS2", "DGS30", "DFII10", "BREAKEVEN")), ("FOMC 정책 환경", "경기침체 해석은 조건부 배경으로만 확인"), cautions=("10Y−3M을 경기침체 확률로 바꾸지 않습니다.",)),
    "BREAKEVEN": GuideDef("10년 일반 국채와 물가연동국채 금리의 차이입니다. 기대 인플레이션뿐 아니라 인플레이션 위험보상과 유동성 영향도 포함합니다.", "명목 장기금리 움직임에 일반·물가연동 국채금리 차이가 함께 움직이는지 봅니다.", ("DFII10", "DGS30", "TERMPREM"), _notes(("DFII10", "DGS30", "TERMPREM")), ("물가 지표와 인플레이션 위험 배경", "FOMC의 물가 관련 커뮤니케이션"), cautions=("10Y Breakeven은 순수 기대 인플레이션이 아닙니다.",)),
    "TERMPREM": GuideDef("10Y Term Premium은 장기채를 오래 보유하는 데 필요한 추가 보상에 대한 모델 추정치입니다.", "장기 듀레이션 보상이 장기금리를 끌어올리는 요인으로 움직이는지 확인합니다.", ("DGS30", "DFII10", "BREAKEVEN", "DGS2"), _notes(("DGS30", "DFII10", "BREAKEVEN", "DGS2")), ("국채 수급", "듀레이션 위험", "정책 불확실성"), cautions=("10Y Term Premium은 모델 추정치이며 저장된 30Y 같은만기 분해에 직접 더하는 항목이 아닙니다.",)),
    "HYOAS": GuideDef("HY OAS는 저신용 회사채가 비슷한 만기의 국채보다 추가로 부담하는 금리 격차입니다.", "HY 수준이 높은지와 최근 방향이 확대인지 축소인지 분리해 봅니다.", ("BBBOAS", "AOAS", "HY_BBB", "CPSPREAD", "VIX", "NFCI", "STLFSI"), _notes(("BBBOAS", "AOAS", "HY_BBB", "CPSPREAD", "VIX", "NFCI", "STLFSI")), (), cautions=("HY 수준 상태와 신용 에피소드 상태는 같은 라벨 체계가 아닙니다.", "OAS는 총 회사채 금리가 아니라 국채 대비 추가 금리입니다.")),
    "VIX": GuideDef("VIX는 S&P 500 옵션 가격에 반영된 주식시장 예상 변동성 지수입니다.", "주식시장 변동성이 회사채 스프레드와 함께 움직이는지, 또는 변동성만 따로 움직이는지 봅니다.", ("HYOAS", "BBBOAS", "CPSPREAD", "NFCI", "STLFSI"), _notes(("HYOAS", "BBBOAS", "CPSPREAD", "NFCI", "STLFSI")), (), cautions=("VIX 상승은 신용 스트레스의 증거가 아니라 주식시장 변동성의 변화입니다.",)),
    "NFCI": GuideDef("NFCI는 여러 금융시장과 금융기관 지표를 묶은 주간 금융여건 참고 지수입니다.", "개별 신용·변동성 지표가 넓은 금융여건 지표와 함께 움직이는지 참고합니다.", ("HYOAS", "BBBOAS", "AOAS", "CPSPREAD", "VIX"), _notes(("HYOAS", "BBBOAS", "AOAS", "CPSPREAD", "VIX")), (), cautions=("주간 복합지표라 관측일이 다를 수 있고 구성요소 중복과 수정 위험이 있습니다. 엔진 입력이나 새 종합 점수로 쓰지 않습니다.",)),
    "STLFSI": GuideDef("STLFSI는 여러 금융시장 스트레스 지표를 묶은 주간 참고 지수입니다.", "개별 신용·변동성 지표가 넓은 스트레스 지표와 함께 움직이는지 참고합니다.", ("HYOAS", "BBBOAS", "AOAS", "CPSPREAD", "VIX"), _notes(("HYOAS", "BBBOAS", "AOAS", "CPSPREAD", "VIX")), (), cautions=("주간 복합지표라 관측일이 다를 수 있고 구성요소 중복과 수정 위험이 있습니다. 엔진 입력이나 새 종합 점수로 쓰지 않습니다.",)),
}
...
def _hy_bbb_row(data_quality: dict | None) -> dict[str, Any] | None:
    ...
    if not lens:
        return None
    return {"key": "HY_BBB", "latest_value": lens.get("latest_value_bp"), "value_unit": "bp", "change_1m": lens.get("change_1m_bp"), "change_unit": "bp", "direction": _dir_from_change(lens.get("change_1m_bp")), "state": lens.get("label"), "latest_date": lens.get("observed_date") or lens.get("latest_date") or "신용 에피소드 렌즈 기준"}
...
def current_data_section(key: str, row: Any, *, data_quality: dict | None = None) -> str:
    ...
    return "\n".join(["## 현재 데이터", "", "| 항목 | 현재 |", "|---|---|", f"| 최신값 | {val} |", f"| 약 1개월 변화 | {chg} |", f"| 현재 상태/방향 | {st} |", f"| 관측일·자료상태 | {date} |"])
...
def guide_markdown(key: str, row: Any, *, matrix: pd.DataFrame | None = None, aux_df: pd.DataFrame | None = None, data_quality: dict | None = None, one_line: str = "") -> str:
