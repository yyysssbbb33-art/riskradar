...
RiskRadar의 메인 화면은 `Market Stress Board`입니다. 먼저 HY OAS와 VIX를 같이 보고,
...
- `20obs/60obs`: 달력 일수가 아니라 실제 관측치 20개·60개 전 대비 변화량. 대략 1개월·3개월 변화로 해석.
- `5Y%/10Y%`: 과거 5년·10년 범위에서 현재값이 어느 위치인지 보는 백분위. 90%면 역사적으로 높은 구간.
- `급락`: 금리 지표가 짧은 기간에 크게 하락했는지 표시. 완화 기대일 수도, 경기둔화 공포일 수도 있음.
- `근거`: 상태 라벨이 붙은 이유를 한 문장으로 요약.

### 30D History 탭

`30D History`는 HF Dataset에 저장된 과거 `versions/<cache_version>/signal_matrix.parquet`를 모아서
최근 30일 동안 각 지표가 어떻게 변했는지 보여주는 탭입니다. 선택 상자에서 VIX, HY OAS, T10Y3M,
30Y, 2Y, 10Y Real 중 하나를 고르면 날짜별 최신값·상태·20obs/60obs 변화·백분위·근거를 볼 수 있습니다.
